...
class SpikeDetector:
    ...
    def detect_spikes(self, data: List[float], threshold: float = 2) -> List[Tuple[int, float]]:
    ...
    def scan_account(self, start_time: datetime, end_time: datetime, threshold: float = 2) -> List[Dict[str, Any]]:
        ...
        for i, metric in enumerate(metrics):
            ...
            logger.info(f"Analyzing metric {i+1}/{len(metrics)}: {namespace}:{metric_name}")
            data = self.get_metric_data(namespace, metric_name, dimensions, start_time, end_time)
    ...
    def generate_report(self, start_time: datetime, end_time: datetime, threshold: float = 2) -> List[Dict[str, Any]]:
        """Generate a report of detected spikes across the account."""
        logger.info("Generating spike detection report")
        report = self.scan_account(start_time, end_time, threshold)
