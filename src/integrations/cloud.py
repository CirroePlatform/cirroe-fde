...
class CloudIntegration:
    ...
    def _get_credentials(self) -> Dict[str, Dict[str, str]]:
        ...
        """
        try:
            result = self.supa_client.table("cloud_credentials").select("*").eq("org_id", self.org_id).execute()
    ...
    def _set_temp_credentials(self, provider: str) -> None:
        ...
        if provider == 'aws':
            ...
            if 'session_token' in self.credentials['aws']:
                os.environ['AWS_SESSION_TOKEN'] = self.credentials['aws']['session_token']
                
        elif provider == 'azure':
            os.environ['AZURE_CLIENT_ID'] = self.credentials['azure']['client_id']
            os.environ['AZURE_CLIENT_SECRET'] = self.credentials['azure']['client_secret']
    ...
    def execute_command(self, provider: str, command: str) -> Dict[str, Any]:
        ...
        if provider not in ['aws', 'azure', 'gcp']:
            raise ValueError("Unsupported cloud provider. Choose from 'aws', 'azure', or 'gcp'.")

        try:
            self._set_temp_credentials(provider)
            
            if provider == 'aws':
                result = subprocess.run(['aws'] + command.split(), capture_output=True, text=True, check=True)
            elif provider == 'azure':
                result = subprocess.run(['az'] + command.split(), capture_output=True, text=True, check=True)
            elif provider == 'gcp':
                result = subprocess.run(['gcloud'] + command.split(), capture_output=True, text=True, check=True)
